...
export class DatabaseService {
    ...
    private getDb() {
        return this.sqlite.create({ name: 'ponto.db', location: 'default' });
    }
        ...
        db.executeSql('select * from db_version', {})
            .then((data) => {
                if (data.rows.length == 0) {
                 ...
                 //   var registrarPonto = 'insert into ponto (entrada) values (?)';
    ...
    private getDatabaseSchema() {
        ...
        tables.push(
            new DatabaseTable("ponto",
                [
                    new DatabaseColumn("id", "integer", false),
                    new DatabaseColumn("Data", "date", false),
                    new DatabaseColumn("entrada", "integer", false),
                    new DatabaseColumn("intervalo", "integer", false),
                    new DatabaseColumn("retorno", "integer", false),
                    new DatabaseColumn("saida", "integer", false),
                    new DatabaseColumn("extra", "integer", false)
                ]));
